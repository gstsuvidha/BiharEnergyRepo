...
{
    public class Gstr1RepositoryForAccountingUnit : IGstr1RepositoryForAccountingUnit
    {
        private readonly ApplicationDbContext _context;
        private readonly IQueryModelDatabase _database;

        public Gstr1RepositoryForAccountingUnit(ApplicationDbContext context, IQueryModelDatabase database)
        {
            _context = context;
            _database = database;
        }

        public IEnumerable<Gstr1B2B> GetAllB2BInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {

            return _database.SalesInvoicesFor(accountingUnitId)
        ...
        public async Task<IEnumerable<GetCustomerName>> GetCustomerNameAtB2B(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
            var query = _context.SalesInvoices.Include(c => c.Customer);

            return await query.Where(cus => cus.AccountingUnitId == accountingUnitId && cus.Date >= frmDate && cus.Date <= toDate).Select(cu => new GetCustomerName
            {
        ...
        public IEnumerable<Gstr1B2Cl> GetB2ClInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
            return _database.SalesInvoicesFor(accountingUnitId)
                                .ForDateRange(frmDate, toDate)
        ...
        public IEnumerable<Gstr1B2C> GetAllB2CInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
        ...
        public IEnumerable<Gstr1CreditDebitNoteRegistered> GetAllCdnRInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
            return _database.IssueNotesFor(accountingUnitId)
                                    .ForDateRange(frmDate, toDate)
                                    .ForInvoiceCategory(InvoiceCategory.B2B)
        ...
        public IEnumerable<PlaceOfSupplyCdnr> GetPlaceOfSupplyAtCdnr(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
        ...
        public IEnumerable<Gstr1CreditDebitNoteUnregistered> GetAllCdnUrInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
        ...
        public IEnumerable<PlaceOfSupplyCdnur> GetPlaceOfSupplyAtCdnur(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
        ...
        public IEnumerable<Gstr1AdvanceReceivedResources> GetAllAdvanceReceives(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {

            return _database.AdvanceReceivesFor(accountingUnitId)
                                    .ForDateRange(frmDate, toDate)
        ...
        public async Task<TaxSummary> GetB2BTotalDashboardSummary(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {

            return await _database.SalesInvoicesFor(accountingUnitId)
                                    .ForDateRange(frmDate, toDate)
                                    .ForInvoiceCategory(InvoiceCategory.B2B)
                                    .GroupBy(si => 1)
        ...
        public async Task<IEnumerable<HsnReporting>> GetAllHsnReport(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
            var query = _context.SalesInvoiceItems.Include(si => si.SalesInvoice).Include(pro => pro.Product);

            return await query.Where(si => si.SalesInvoice.AccountingUnitId == accountingUnitId
        ...
        public async Task<IEnumerable<Gstr1DocumentDetail>> GetDocumentDetail(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {
            var query = _context.SalesInvoices.GroupBy(si => 1);

            var maximumInvoiceNumber = _context.SalesInvoices.Where(si => si.AccountingUnitId == accountingUnitId
                                           && si.Date >= frmDate
                                           && si.Date <= toDate).Max(inv => inv.InvoiceNumber);


            var minimumInvoiceNumber = _context.SalesInvoices.Where(si => si.AccountingUnitId == accountingUnitId
                                           && si.Date >= frmDate
                                           && si.Date <= toDate).Min(inv => inv.InvoiceNumber);


            var countInvoiceNumber = _context.SalesInvoices.Where(si => si.AccountingUnitId == accountingUnitId
                                           && si.Date >= frmDate
        ...
        public async Task<IEnumerable<ExportReportingResource>> GetAllExportInvoices(DateTime frmDate, DateTime toDate, string accountingUnitId)
        {

            var query = _context.ExportInvoices.Include(i => i.ExportInvoiceItems);

            return await query.Where(exp => exp.AccountingUnitId == accountingUnitId && exp.Date >= frmDate && exp.Date <= toDate).Select(expt => new ExportReportingResource
            {
